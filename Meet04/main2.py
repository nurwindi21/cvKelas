...
def apply_gradient(image_array):
    ...
    gradient_array = np.zeros_like(image_array)

    for x in range(width):
        brightness = int(255 * (x / width))  # Calculate brightness from 0 to 255
        gradient_array[:, x] = brightness  # Apply the brightness to the entire column
...
def main():
    ...
    combined_array = np.clip(image_array + gradient_array, 0, 255)

    # Convert the combined array back to an image
    gradient_image = Image.fromarray(np.uint8(combined_array))
    
    # Save the gradient image in the current working directory with a fixed name
    gradient_image.save('GradientImage2.png')
